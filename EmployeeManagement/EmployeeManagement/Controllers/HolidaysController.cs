    ...
    public class HolidaysController : Controller
    {
        ...
        public async Task<IActionResult> Index(HolidayViewModel vm)
        {
            var holidays = _context.Holidays
                ...
                .Include(x => x.ModifiedBy).AsQueryable();
            if (!string.IsNullOrEmpty(vm.Title))
            {
                holidays = holidays
                    .Where(x => x.Title.Contains(vm.Title));
            }
            ...
            {
                holidays = holidays
                    .Where(x => x.StartDate == vm.StartDate);
            }
            if (vm.EndDate != null)
            {
                holidays = holidays
                    .Where(x => x.EndDate == vm.EndDate);
            }
        ...
        public async Task<IActionResult> Create(Holiday holiday)
        {
            ...
            {
                TempData["Error"] = "An error occured while creation the holiday" + ex.Message;
                return View(holiday);
            }
        }
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            var holiday = await _context.Holidays
                .Include(x => x.CreatedBy)
                .Include(x => x.ModifiedBy).FirstOrDefaultAsync(x => x.Id == id);
            if (holiday == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> Edit(int id, Holiday holiday)
        {
            ...
            try
            {
                ...
                TempData["Message"] = "Holiday Updated Successful!";
                await _context.SaveChangesAsync(Userid);
            }
